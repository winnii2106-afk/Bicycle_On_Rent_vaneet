...
exports.getAllRenter = async (req, resp) => {
    req.body.userType = 2
    await User.find(req.body).then(res => {
        resp.send({ success: true, status: 200, message: "All Renters loaded", data: res })
    }).catch(err => {
        resp.send({ success: false, status: 500, message: !!err.message ? err.message : err })
    })
}
exports.getAllHirer = async (req, resp) => {
    req.body.userType = 3
    await User.find(req.body).then(res => {
        resp.send({ success: true, status: 200, message: "All Hirers loaded", data: res })
    }).catch(err => {
        resp.send({ success: false, status: 500, message: !!err.message ? err.message : err })
    })
}
...
exports.addUser = async (req, resp) => {
    ...
    if (!!validation)
        ...
        let UserData = {
            ...
            userType: !!formData.userType ? formData.userType : 3
        ...
        if (prevUser)
            ...
            if (!!formData.image) {
                user.idProof_image = "idProof/" + formData.image
            }
            user.userStatus = user.userType == 3 ? 2 : 1
            user.save().then(res => {
    ...
    if (!!validation)
        ...
        await User.findOne(query).then(async res => {
            if (!!res) {
                ...
                if (!!formData.image)
                    res.idProof_image = "idProof/" + formData.image
                if (!!formData.userStatus)
                    res.userStatus = formData.userStatus
